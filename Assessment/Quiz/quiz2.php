...
/**
 *                           Quiz 2
 *       Link: https://cislinux.hfcc.edu/~alsakkinen/cis222Repo/Homework/Quiz2.php
 *       Email: alsakkinen@hawkmail.hfcc.edu
 *       Written by Amber Sakkinen, September 12th, 2019
 *       Last edited: September 12th, 2019
 *
 *
 *
 * q2.txt
...
//		Then use a foreach loop to step through that array and echo their contents, don't forget to format with <br> tags.
    $classes = array("C Programming", "SQL Databases", "ASP.Net", "Mobile App Development");

    foreach($classes as $class)
    {
        echo $class[0]++ <br>; // I know this is probably terrible but I gave it my best shot.
        echo $class . '<br>'; // Pretty close, class will update with the values in classes, so you don't need to ref 
...
//		Finish the form below so that it contains a text input to enter a users email address,
//      and a submit button that posts the data to a add_email_subscriber.php page.
//		You do not need to handle the data or response of the form, just create the form itself.
?>
    <form><!-- need the add_email_subscriber.php action here -->
        <input type="hidden" name="action" value="addToMailingList" >
        <p>Please enter user's email address: </p>
        <textinput></textinput>
        <button post="add_email_subsriber.php">Submit</button>
    </form>
<?php
